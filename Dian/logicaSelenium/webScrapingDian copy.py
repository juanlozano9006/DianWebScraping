...
class controlMCode():
    ...
    def __init__(self,  baseparametrica = '', usuarioD = '', passwordD = '', proceso = 1, tipo = '', fechainicial = '', fechafinal = ''):
        ...
        # self.rutaarchivos = self.CrearCarpeta(os.path.join(rutainicial, 'archivos')) 
        # prefs = {'download.default_directory' : self.rutaarchivos, 'safebrowsing.enabled': 'false'}
        ...
        self.driverCartera.implicitly_wait(90)
    ...
    # Imprime el texto del elemento que es inicio 
            # print(element.text)
            ...
            enlace_zip = self.driverCartera.find_element(
                # By.XPATH, "//table[@id='tableExport']/tbody/tr[@class='odd'][1]/td[@class='text-left'][3]/a"
    ...
    def descargarFacturas(self,df ):
        try:
            ...
            for index, row in df.iterrows():
                # print(row['CUFE/CUDE'])
                #busca el campo de cufe del formulario de la dian
                self.waitChange()
                campocufe = self.driverCartera.find_element(By.XPATH, "//div[@class='col-md-12 padding-horizontal-0']/div[@id='document-list']/div[@class='panel-body padding-20']/div[@id='divFirstRowDocFilter']/div[@class='col-sm-3 padding-xs-0'][1]/div[@class='form-group']/input[@id='DocumentKey']")
                campocufe.clear()
                time.sleep(1)
                ...
                while True:
                    try:
                        # Espera hasta que el elemento esté presente (máximo 10 segundos por ciclo)
                        element = WebDriverWait(self.driverCartera, 10).until(
                            EC.presence_of_element_located(
                                (By.XPATH, "//div[@class='panel-body']/div[@class='table-responsive padding-bottom-15']/div[@id='tableDocuments_wrapper']/table[@id='tableDocuments']/tbody/tr[@class='odd document-row']/td[5]")
                            )
                        )
                        if element.text == prefijo:
                            break
                    except TimeoutException:
                        pass  # Si no aparece, sigue esperando hasta el tiempo máximo del counter

                    self.waitChange()
                    time.sleep(0.9)
                    counter += 0.6
                    if counter >= 40:  # Si han pasado 60 segundos
                        break
                self.waitChange()
                time.sleep(1)
                ...
                self.enviar_correoAwsSes(asunto, zip_bytes, prefijo)
